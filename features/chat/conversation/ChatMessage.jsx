...
// Freshly-sent audio can 404/fail to decode for a moment while the file
// finishes propagating on the storage backend; the <audio> element doesn't
// auto-retry once it errors, so we force one reload after a short delay.
...
// One row of the conversation: its optional date separator, the bubble with its
// select/delete affordances, and its optional timestamp.
const ChatMessage = ({
  msg,
  isCurrentUser,
  date,
  ...
  // The select/delete affordances below are gated on isCurrentUser: the API
  // only accepts deletions of your own messages.
