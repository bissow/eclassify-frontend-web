...
// Gate only — contains no hooks other than the store read, so its hook order can
// never change. MessageComposer mounts only when sending is actually allowed,
// which means the composer's hooks are unconditional and its draft state is
// dropped when the chat becomes unsendable (ad closed, blocked mid-conversation).
const SendMessage = () => {
  ...
  return <MessageComposer chatId={selectedChatDetails?.id} />;
};
