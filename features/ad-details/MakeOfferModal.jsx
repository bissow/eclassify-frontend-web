...
import { itemOfferApi, tipsApi } from "@/lib/api";
import { useTranslation } from "@/lang/useTranslation";
import { Skeleton } from "@/components/ui/skeleton";
import { CheckIcon } from "@phosphor-icons/react";
import { useNavigate } from "@/hooks/useNavigate";
import { groupPrice, ungroupPrice } from "@/lib/format";
import { cn } from "@/lib/utils";

const MakeOfferModal = ({ isOpen, onClose, productDetails }) => {
  const { t } = useTranslation();
  const { navigate } = useNavigate();
  const itemCurrency = productDetails?.currency;
  const displaySymbol = itemCurrency?.symbol;
  const displayPosition = itemCurrency?.symbol_position;
  ...
  const validateOffer = () => {
    ...
    const sellerPrice = Number(productDetails?.price);
  ...
  const handleChange = (e) => {
    const raw = ungroupPrice(e.target.value, itemCurrency);
    if (raw === null) return; // malformed input (e.g. bad paste) — ignore keystroke
    setOfferAmount(raw);
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              type="text"
              inputMode="decimal"
              value={groupPrice(offerAmount, itemCurrency)}
